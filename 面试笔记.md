...
![image-20240326131049665](https://md-jomo.oss-cn-guangzhou.aliyuncs.com/IMG/image-20240326131049665.png)

![image-20240326131814802](https://md-jomo.oss-cn-guangzhou.aliyuncs.com/IMG/image-20240326131814802.png)
...
public:
    ...
    int kInversePairs(int n, int k) {
        // dp[i][j]:使用数字1,2,...,i的排列构成长度为i的数组，并且恰好包含j个逆序对的方案数
        vector<vector<long>> dp(2, vector<long>(k + 1, 0));
        dp[1][0] = 1;

        for (int i = 2; i <= n; i++) {
            // 对于i个数的序列，最多只会有i * (i - 1) / 2个逆序对
            for (int j = 0; j <= min(k, i * (i - 1) / 2); j++) {
                dp[i & 1][j] =
                    ((j >= 1 ? dp[i & 1][j - 1] : 0) + dp[(i - 1) & 1][j] -
...
- `传输层：`取出TCP头或UDP头，根据四元组(源IP、源端口、目的IP、目的端口)作为标识，找出对应的Socket，并把数据放松Socket的接收缓冲区
